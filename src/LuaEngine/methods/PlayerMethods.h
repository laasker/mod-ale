...
namespace LuaPlayer
{
    ...
    {
        ALE::Push(L, player->InArenaQueue());
        return 1;
    }
    ...
    */
    ...
    {
        uint8 button = ALE::CHECKVAL<uint8>(L, 2);
        uint32 action = ALE::CHECKVAL<uint32>(L, 3);
        uint8 type = ALE::CHECKVAL<uint8>(L, 4);
    ...
    // nao sei se é necessario ainda, vou testar dps
